...
func (c *CombatStats) Decode(holder *cffi.CFFIValueClass, typeMap baml.TypeMap) {
	...
	for _, field := range holder.Fields {
		...
		case "enemy_units_killed":
			c.Enemy_units_killed = baml.Decode(valueHolder).Int()

		case "enemy_buildings_destroyed":
			c.Enemy_buildings_destroyed = baml.Decode(valueHolder).Int()

		case "enemy_units_presumed_killed":
			c.Enemy_units_presumed_killed = baml.Decode(valueHolder).Int()
...
type Doctrine struct {
	...
	Force_size                     float64  `json:"force_size"`
}
...
func (c *Doctrine) Decode(holder *cffi.CFFIValueClass, typeMap baml.TypeMap) {
	...
	for _, field := range holder.Fields {
		...
		case "force_size":
			c.Force_size = baml.Decode(valueHolder).Float()
...
func (c Doctrine) Encode() (*cffi.HostValue, error) {
	...
	fields["force_size"] = c.Force_size
...
func (c *GameSituation) Decode(holder *cffi.CFFIValueClass, typeMap baml.TypeMap) {
	...
	for _, field := range holder.Fields {
		...
		case "harvester_percent":
			c.Harvester_percent = baml.Decode(valueHolder).Int()
