...
rule form-ground-attack {
  ...
  category squad-form
  because "the squad target scales with the force the doctrine wants, because a CONSTANT cannot. ground-attack-group-size is 5 or 6 every game while the army runs from 7 combat units to 27 at peak, so a fixed detachment is by construction a smaller and smaller fraction of it — and what reached the enemy base in game 150 was TWO units, none of them together, out of a squad that formed at a full 6. Every fix behind it worked: the squad forms, holds formation, wins its category and now presses the base, closest approach 0.143 of the map diagonal against 0.244 the game before. It arrives as a pair because it was only ever six. Multiplying by force-scale gives 10 to 14 for the doctrines the strategist picks, which is most of the army rather than a detachment from it. The threshold is how many unassigned idle ground units Vimy ever HAS at one instant, not what a war party ought to number. At six tenths of a group size the strategist pins near eleven, forming demanded six simultaneously idle — and unassigned-idle-ground ran a median of 0 and a maximum of 6 across games 127 and 129, so the gate held in 0 of 1364 sampled states. Not rarely: never. Three tenths asks for three and holds in about one state in ten. The reason idle is always near zero is that defensive rules repossess the army continuously — emergency base defense fired 42 times and recall-stray-units 37 in one game — so a rendezvous condition counted on simultaneity was the wrong shape for this army whatever number it carried"
  do form-squad(ground-attack, Ground, trunc(ground-attack-group-size * force-scale()), Attack)
